...
genai.configure(api_key="AIzaSyAVO8ufZf9k9ZtrLgeUbia5WE-YQTOZ55c")
...
def generate_gemini_response(input_prompt, uploaded_files, selected_image_index, question_prompt):
    image_prompt, temp_path = input_image_setup(
...
def main():
    st.title("Bills Query")

    # File Upload
    st.sidebar.header("Upload Image or Images")
    uploaded_files = st.sidebar.file_uploader("Choose images...", type=[
                                              "jpg", "png"], accept_multiple_files=True, key="file_uploader")
    ...
    if st.button("Generate Response"):
        try:
            response_text = generate_gemini_response(
                input_prompt, uploaded_files, file_names.index(selected_image_index), question_prompt)
